	...
	public partial class MainMenu : Form
	{
		...
		private void getOrderNumber()
		{
			...
			try
			{
				...
				dailyOrderNumberLabel.Text = (maxOrderNumber + 1).ToString();
			}
		...
		private void acceptOrder(string orderType)
		{
			...
			string[] ticketTypes = { "Customer", "Kitchen" };
			...
			// clear everything
			clearMenu();
			dailyOrderNumberLabel.Text = (Convert.ToInt32(dailyOrderNumberLabel.Text) + 1).ToString(); // increment order number
			con.Close();
		}
		...
		private void addOrderToOrderTbl(string orderType)
		{
			...
			addOrderToDatabase.Parameters.AddWithValue("@DON", dailyOrderNumberLabel.Text);
		...
		private decimal getTotalPrice() // check if paying for counter or for collection/delivery lmao
		{
			decimal totalPrice;
			if (viewOrdersPricePanel.Visible == false) // counter 
			{
		...
		private void paymentAcceptButton_Click(object sender, EventArgs e)
		{
			...
			else // getting payment for a collection/delivery
			{
				...
				acceptPayment.Parameters.AddWithValue("@DON", viewOrdersSelectedOrderDailyOrderNumber);
		...
		int viewOrdersSelectedOrderDailyOrderNumber = -1; // id of currently selected order in ordersdatagridview
		...
		private void viewOrdersDataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (selectedRowIndex > -1) // just in case u click the header
			{
				...
				viewOrdersSelectedOrderDailyOrderNumber = Convert.ToInt32(viewOrdersDataGridView.Rows[selectedRowIndex].Cells["dailyOrderNumber"].Value);
				DataRow order = viewOrdersDataTable.Select($"dailyOrderNumber = '{viewOrdersSelectedOrderDailyOrderNumber}'")[0];
				// fill in order type
				...
				DataView singleOrderDataView = new DataView(viewOrdersOrderItemsDictionary[viewOrdersSelectedOrderDailyOrderNumber]);
		...
		{
			if (viewOrdersSelectedOrderDailyOrderNumber != -1) // if theres a selected order
			{
				con.Open();
				printDocument = new PrintDocument();
